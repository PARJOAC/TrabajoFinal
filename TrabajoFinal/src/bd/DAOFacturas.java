		...
		try (Connection conn = BDConnection.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				while (rs.next()) {
					...
					String detalle = String.format("%s - %.2f € x %d = %.2f €", rs.getString("nombreProducto"),
							rs.getDouble("precioUnitario"), rs.getInt("cantidad"), rs.getDouble("subtotal"));
					lista.add(detalle);
				}
			}
		}

		// Se retorna la lista de detalles
		return lista;
	}
}
